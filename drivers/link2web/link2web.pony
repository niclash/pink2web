...
actor Link2Web is Driver
  ...
  let _multiplexer:(Link2WebMultiplexer | None)
  let _expansions:Map[U8, Link2WebExpansionCard] = Map[U8,Link2WebExpansionCard]
  ...
  be register(slot:U8, expansion:Link2WebExpansionCard) =>
...
interface val Link2WebExpansionCardNotify
  fun val notify( value:Linkable )

trait tag Link2WebExpansionCard
  be update()
