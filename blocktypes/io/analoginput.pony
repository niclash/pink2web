...
class val AnalogInputBlockDescriptor is BlockTypeDescriptor
  let _out:OutputDescriptor

  new val create() =>
      _out = OutputDescriptor("out", "number", "Logical Value that can be mapped to physical output", false )
  ...
  fun val description(): String =>
    "Logical Analog Input, which can be mapped to a physical input in the runtime environment."
...
class val AnalogInputBlockFactory is BlockFactory
  let _descriptor: AnalogInputBlockDescriptor val = AnalogInputBlockDescriptor
  ...
  fun create_block( instance_name: String, context:SystemContext val, x:F64, y:F64):Block =>
    AnalogInput( instance_name, _descriptor, context, x, y )
