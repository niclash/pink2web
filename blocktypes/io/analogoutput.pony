...
class val AnalogOutputBlockDescriptor is BlockTypeDescriptor
  let _input:InputDescriptor

  new val create() =>
      _input = InputDescriptor("input", "number", "Logical Value that can be mapped to physical output", false )
  ...
  fun val description(): String =>
    "Logical Analog Output, which can be mapped to a physical output in the runtime environment."
...
class val AnalogOutputBlockFactory is BlockFactory
  let _descriptor: AnalogOutputBlockDescriptor val = AnalogOutputBlockDescriptor
  ...
  fun create_block( instance_name: String, context:SystemContext val, x:F64, y:F64):Block =>
    context(Fine) and context.log(Fine, "create AnalogOutput")
    AnalogOutput( instance_name, _descriptor, context, x, y )
