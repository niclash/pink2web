...
actor Graphs
  ...
  be report_error( graph:String, type':String, message: String ) =>
  ...
  be _status(id:String, name:String, descr:String, uptime:I64, running:Bool, started:Bool, debug:Bool) =>
    _context(Info) and _context.log(Info, "status: " + id + " : " + uptime.string() + " : " + started.string() + " : " + running.string() + " : " + debug.string() )
    for s in _subscribers.values() do
      s.status( id, name, descr, uptime, started, running, debug)
    end

  be rename_graph( id:String, old_name:String, new_name:String ) =>
    try
      _graphs_by_id(id)?.rename(old_name, new_name)
    else
