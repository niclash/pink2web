## Changes
* 2024-03-18:
  - Adding [`graph:list`](#graph-list) and [`graph:get`](#graph-get)
* 2024-03-16:
  - Pink2Web forks the [flowbased/fbp-protocol](https://github.com/flowbased/fbp-protocol)
* 2019-10-09:
...
* 2018-03-26:
  - Fixed documentation for [`component:setsource`](#component:setsource) to use [`component:source`](#component:source) input, and [`component:component`](#component-component) output
  - Added schema for [`trace:error`](#trace-error) message
  - Added `:error` output to all capabilities where user can perform actions that may fail
* 2018-03-23:
  - Added optional `graph` key to [`network:error`](#network-error) payloads
* 2018-03-22: **Version 0.7**
  - Added [`network:debug`](#network-debug) and [`network:getstatus`](#network-getstatus) to the [`network:control`](#network-control) permission
* 2018-03-21:
  - Fixed signature of [`runtime:packet.payload`](#runtime-packet), [`runtime:packetsent.payload`](#runtime-packetsent), and port definition `default` to accept any payload type
  - Added `values` and `default` keys for port definitions
  - Added schema for [`component:componentsready`](#component-componentsready) output message
  - Added schema for [`graph:clear`](#graph:clear) output message
  - Added `packetsent` response for [`runtime:packet`](#runtime-packet) input message
* 2017-09-17:
...
* 2017-05-04:
  - Fixed protocol errors ([`graph:error`](#graph-error), [`component:error`](#component-error) and [`runtime:error`](#runtime-error)) to have mandatory `message` string payload.
  - Fixed missing `required` markers in some JSON schemas for `graph` protocol. Affected messages:
[`graph:renamegroup`](#graph-renamegroup), [`graph:renameinport`](#graph-renameinport), [`graph:removeinport`](#graph-removeinport), [`graph:addinitial`](#graph-addinitial), [`graph:changeedge`](#graph-changenode)
  - More readable HTML output, including property value types and examples
* 2017-05-03:
  - Added more optional metadata to [`runtime:runtime`](#runtime-runtime) message: `repository`, `repositoryVersion` and `namespace`
* 2017-02-20:
  - Fixed payload definition of [`network:edges`](#network-edges) missing mandatory `graph` key
* 2016-07-01:
  - [`network:error`](#network-error) payload may now contain an optional `stacktrace`
* 2016-06-23:
